#!/usr/bin/env python3
"""Recompute Wine Hard-empty roots and retain geometric/factual follow-up."""
...
SCHEMA = "wine-hard-empty-native-audit-v1"
REASONS = {
...
def audit_run(run_dir: Path, native_library: Path) -> dict[str, object]:
    ...
    for row in frames:
        ...
        roots.append({
            ...
            "all_actions_have_four_frame_collision_witness": all(
                item["first_collision_frame"] is not None
    ...
    gates = {
        ...
        "every_action_has_collision_witness": all(
            row["all_actions_have_four_frame_collision_witness"] for row in roots
        ),
    }
    return {
        "run_dir": str(run_dir),
