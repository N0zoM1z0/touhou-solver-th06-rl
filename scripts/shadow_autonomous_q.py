#!/usr/bin/env python3
"""Replay an immutable autonomous candidate on disjoint factual Wine states."""
...
from th06_rl.autonomous_learning import _object, _transition_rows, _validate_run
...
def shadow(
    ...
    *,
    ...
    if state.get("mode") != "shadow":
        raise ValueError("shadow audit requires a shadow policy state")
            ...
            context = PolicyContext(
                ...
                action_features=tuple(
                    (
                        str(action),
                        tuple(
                            (str(name), float(value)) for name, value in features
                        ),
                    )
    ...
    if observed_groups != expected_groups:
        raise ValueError("shadow runs do not exactly match held-out episode groups")
