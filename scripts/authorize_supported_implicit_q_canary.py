#!/usr/bin/env python3
"""Hash-bind a clean Generation-5 native shadow into active canary state."""
...
from scripts.shadow_supported_implicit_q import SCHEMA as SHADOW_SCHEMA  # noqa: E402
from th06_rl.implicit_learning import STATE_SCHEMA  # noqa: E402
...
def authorize(state_path: Path, shadow_path: Path) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("candidate did not pass Generation-5 fit authorization")
    ...
    if (
        ...
        raise ValueError("Generation-5 native shadow is absent, stale, or invalid")
    state["mode"] = "active"
    authorization["active_canary"] = {
        "schema": "autonomous-generation-5-active-canary-authorization-v1",
