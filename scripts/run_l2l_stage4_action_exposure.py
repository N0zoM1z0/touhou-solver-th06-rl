#!/usr/bin/env python3
"""Collect and audit the frozen L2l 12-root action-exposure pilot."""
...
PREREG_SCHEMA = "th06-rl-l2l-stage4-action-exposure-prereg-v1"
SCHEDULE_SCHEMA = "th06-rl-l2l-stage4-action-exposure-schedule-v1"
COLLECTION_SCHEMA = "th06-rl-l2l-stage4-action-exposure-collection-v1"
RESULT_SCHEMA = "th06-rl-l2l-stage4-action-exposure-result-v1"


def load_prereg(path: Path) -> dict[str, Any]:
    prereg = _object(path.resolve())
    if prereg.get("schema") != PREREG_SCHEMA:
        raise ValueError("L2l preregistration schema mismatch")
    for key in ("collection", "gate", "paths", "sha256_bindings"):
        if not isinstance(prereg.get(key), dict):
            raise ValueError(f"L2l preregistration lacks {key}")
    ...
    if (
        ...
        raise ValueError("L2l collection contract changed")
    if collection.get("lifecycle_interruptions") != [
        ...
        raise ValueError("L2l lifecycle contract changed")
    if collection.get("infrastructure_admission") != {
        ...
        raise ValueError("L2l infrastructure admission changed")
    ...
    for index, row in enumerate(episodes):
        if (
            ...
            raise ValueError("L2l episode seed or split changed")
    expected_gate = {
        ...
        "hit_support_is_retry_or_contract_gate": False,
    ...
    if gate != expected_gate:
        raise ValueError("L2l gate changed")
    if prereg.get("fits_model") is not False or prereg.get("runs_learned_policy") is not False:
        raise ValueError("L2l may not fit or run a learned policy")
    bound_files = (
        "base_policy_state",
    ...
    for key in bound_files:
        ...
        if not source.is_file() or _sha256(source) != bindings.get(key):
            raise ValueError(f"preregistered L2l input differs: {key}")
    ...
    if (
        ...
        raise ValueError("L2l base exposure policy differs")
    source = _object(_repository_path(paths["source_l2k_erratum"]))
    if (
        source.get("corrected_decision")
        != "proceed-action-exposure-training-collection"
        or source.get("pilot_train_data_admitted") is not True
        or source.get("thresholds_changed") is not False
    ):
        raise ValueError("L2l source is not the bounded L2k pass")
    return prereg
...
def build_schedule(
    ...
    *,
    ...
    for raw in prereg["collection"]["episodes"]:
        ...
        name = f"stage4-action-exposure-h12-{index:03d}"
        rows.append({
            **raw,
            "name": name,
...
def _summary_kwargs(prereg: dict[str, Any]) -> dict[str, object]:
    gate = prereg["gate"]
    return {
        ...
        "hit_support_diagnostic_minimum": int(
            gate["hit_support_diagnostic_minimum"]
        ),
    }
...
def run(prereg_path: Path) -> dict[str, object]:
    ...
    if result_path.is_file():
        ...
        if (
            ...
            raise ValueError("completed L2l result differs")
        return result
    ...
    if schedule_path.is_file():
        ...
        if existing != schedule:
            raise ValueError("L2l resume schedule differs")
    else:
        if artifact_root.exists() and any(artifact_root.iterdir()):
            raise ValueError("L2l artifact root lacks its immutable schedule")
    ...
    for row in schedule["episodes"]:
        ...
        if state_path.is_file():
            if _object(state_path) != state:
                raise ValueError(f"L2l episode policy state differs: {state_path}")
        else:
            _atomic_json(state_path, state)
        if _sha256(state_path) != row["policy_state_sha256"]:
            raise ValueError(f"L2l episode policy state hash differs: {state_path}")
    ...
    if collection_path.is_file():
        if _object(collection_path) != collection:
            raise ValueError("L2l collection ledger differs")
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--preregistration",
        type=Path,
        default=REPOSITORY / "experiments/l2l-stage4-action-exposure-v1.json",
    )
